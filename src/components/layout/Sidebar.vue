  ...
  setup() {
    ...
    const updateActiveItem = () => {
      ...
      const pathMap = [
        ...
        { path: "/musicHall", name: "音乐馆" },
        { path: "/comments", name: "精选评论" },
        { path: "/favorites", name: "我喜欢" },
        { path: "/localMusic", name: "本地歌曲" },
        { path: "/downloads", name: "下载歌曲" },
        { path: "/recentPlay", name: "最近播放" },
      ];

      const specialRoutes = [
        { pattern: /^\/artist\/\d+$/, name: "音乐馆" },
        { pattern: /^\/playlist\/\d+$/, name: "推荐" },
        { pattern: /^\/radio\/\d+$/, name: "音乐馆" },
      ];

      for (const routeInfo of specialRoutes) {
        if (routeInfo.pattern.test(currentPath)) {
          activeItem.value = routeInfo.name;
          return;
        }
      }

      for (const routeInfo of pathMap) {
        if (currentPath.startsWith(routeInfo.path)) {
          activeItem.value = routeInfo.name;
          return;
        }
      }
    ...
    const navigateTo = (path, itemText) => {
